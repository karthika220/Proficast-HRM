...
async function testCompleteWorkflow() {
  ...
  try {
    ...
    try {
      ...
      console.log('📊 Current status:', todayRes.data.status || 'No active session');
      
      // If already checked in and not on break, take a break first
      if (todayRes.data.checkIn && !todayRes.data.checkOut && !todayRes.data.breakStart) {
        console.log('📝 Taking a break from current session...');
    ...
    // Step 2: Resume work (check-in again)
    try {
      ...
      console.log('ℹ️ Already working or no break to resume from');
    }
    ...
    try {
      ...
      console.log('ℹ️ Cannot checkout - may already be checked out');
    }
